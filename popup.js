document.addEventListener('DOMContentLoaded', () => {
  // --- Constants ---
  const TRASH_ICON_SVG = `<svg viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M5.11686 7.7517C5.53016 7.72415 5.88754 8.03685 5.91509 8.45015L6.37503 15.3493C6.46489 16.6971 6.52892 17.6349 6.66948 18.3406C6.80583 19.025 6.99617 19.3873 7.26958 19.6431C7.54299 19.8989 7.91715 20.0647 8.60915 20.1552C9.32255 20.2485 10.2626 20.25 11.6134 20.25H12.3868C13.7376 20.25 14.6776 20.2485 15.391 20.1552C16.083 20.0647 16.4572 19.8989 16.7306 19.6431C17.004 19.3873 17.1943 19.025 17.3307 18.3406C17.4713 17.6349 17.5353 16.6971 17.6251 15.3493L18.0851 8.45015C18.1126 8.03685 18.47 7.72415 18.8833 7.7517C19.2966 7.77925 19.6093 8.13663 19.5818 8.54993L19.1183 15.5017C19.0328 16.7844 18.9638 17.8206 18.8018 18.6336C18.6334 19.4789 18.347 20.185 17.7554 20.7385C17.1638 21.2919 16.4402 21.5308 15.5856 21.6425C14.7635 21.7501 13.7251 21.7501 12.4395 21.75H11.5607C10.2751 21.7501 9.23664 21.7501 8.4146 21.6425C7.55995 21.5308 6.8364 21.2919 6.2448 20.7385C5.65321 20.185 5.36679 19.4789 5.19839 18.6336C5.03642 17.8205 4.96736 16.7844 4.88186 15.5017L4.41841 8.54993C4.39086 8.13663 4.70357 7.77925 5.11686 7.7517Z" /><path fill-rule="evenodd" clip-rule="evenodd" d="M10.3553 2.25004L10.3094 2.25002C10.093 2.24988 9.90445 2.24976 9.72643 2.27819C9.02313 2.39049 8.41453 2.82915 8.08559 3.46084C8.00232 3.62074 7.94282 3.79964 7.87452 4.00496L7.86 4.04858L7.76291 4.33984C7.74392 4.39681 7.73863 4.41251 7.73402 4.42524C7.55891 4.90936 7.10488 5.23659 6.59023 5.24964C6.5767 5.24998 6.56013 5.25004 6.50008 5.25004H3.5C3.08579 5.25004 2.75 5.58582 2.75 6.00004C2.75 6.41425 3.08579 6.75004 3.5 6.75004L6.50865 6.75004L6.52539 6.75004H17.4748L17.4915 6.75004L20.5001 6.75004C20.9143 6.75004 21.2501 6.41425 21.2501 6.00004C21.2501 5.58582 20.9143 5.25004 20.5001 5.25004H17.5001C17.44 5.25004 17.4235 5.24998 17.4099 5.24964C16.8953 5.23659 16.4413 4.90933 16.2661 4.42522C16.2616 4.41258 16.2562 4.39653 16.2373 4.33984L16.1402 4.04858L16.1256 4.00494C16.0573 3.79961 15.9978 3.62073 15.9146 3.46084C15.5856 2.82915 14.977 2.39049 14.2737 2.27819C14.0957 2.24976 13.9072 2.24988 13.6908 2.25002L13.6448 2.25004H10.3553ZM9.14458 4.93548C9.10531 5.04404 9.05966 5.14902 9.00815 5.25004H14.992C14.9405 5.14902 14.8949 5.04405 14.8556 4.9355L14.8169 4.82216L14.7171 4.52292C14.626 4.2494 14.605 4.19363 14.5842 4.15364C14.4745 3.94307 14.2716 3.79686 14.0372 3.75942C13.9927 3.75231 13.9331 3.75004 13.6448 3.75004H10.3553C10.067 3.75004 10.0075 3.75231 9.96296 3.75942C9.72853 3.79686 9.52566 3.94307 9.41601 4.15364C9.39519 4.19363 9.37419 4.24942 9.28302 4.52292L9.18322 4.82234C9.1682 4.86742 9.1565 4.90251 9.14458 4.93548Z" /></svg>`;

  // --- Element Declarations ---
  const urlListContainer = document.getElementById('urlList');
  const newUrlInput = document.getElementById('newUrl');
  const addUrlButton = document.getElementById('addUrl');
  const addCurrentUrlButton = document.getElementById('addCurrentUrl');
  
  const minIntervalInput = document.getElementById('minInterval');
  const maxIntervalInput = document.getElementById('maxInterval');
  const pinnedOnlyCheckbox = document.getElementById('pinnedOnly');
  const enabledCheckbox = document.getElementById('enabled');
  const saveButton = document.getElementById('save');

  // Randomizer Mode Elements
  const randomizerEnabledCheckbox = document.getElementById('randomizerEnabled');
  const randomUrlSection = document.getElementById('randomUrlSection');
  const randomUrlDisplayList = document.getElementById('randomUrlDisplayList');
  const newRandomUrlTextarea = document.getElementById('newRandomUrl');
  const addRandomUrlButton = document.getElementById('addRandomUrl');
  const addCurrentRandomUrlButton = document.getElementById('addCurrentRandomUrl');

  // --- State Variables ---
  let targetUrls = [];
  let randomUrls = [];
  let savePopupTimeout;

  document.getElementById("year").textContent = new Date().getFullYear();

  // --- UI Logic ---
  function toggleRandomizerUI(isRandomizerEnabled) {
    randomUrlSection.style.display = isRandomizerEnabled ? 'block' : 'none';
    pinnedOnlyCheckbox.checked = isRandomizerEnabled;
    pinnedOnlyCheckbox.disabled = isRandomizerEnabled;
  }
  
  function renderUrlList(container, urlArray) {
    container.innerHTML = '';
    urlArray.forEach((url, index) => {
      const li = document.createElement('li');
      const urlSpan = document.createElement('span');
      urlSpan.textContent = url;
      urlSpan.title = url;
      li.appendChild(urlSpan);

      const removeButton = document.createElement('button');
      removeButton.className = 'remove-btn';
      removeButton.title = 'Remove URL';
      removeButton.innerHTML = TRASH_ICON_SVG;
      removeButton.addEventListener('click', () => {
        urlArray.splice(index, 1);
        renderUrlList(container, urlArray);
      });
      li.appendChild(removeButton);
      container.appendChild(li);
    });
  }

  // --- Data Loading & Saving ---
  function loadSettings() {
    const settingsToGet = ['urls', 'minInterval', 'maxInterval', 'pinnedOnly', 'enabled', 'randomizerEnabled', 'randomUrlList'];
    chrome.storage.sync.get(settingsToGet, (settings) => {
      targetUrls = settings.urls || [];
      randomUrls = settings.randomUrlList || [];
      
      renderUrlList(urlListContainer, targetUrls);
      renderUrlList(randomUrlDisplayList, randomUrls);

      minIntervalInput.value = settings.minInterval || 60;
      maxIntervalInput.value = settings.maxInterval || 120;
      
      const isRandomizer = settings.randomizerEnabled || false;
      randomizerEnabledCheckbox.checked = isRandomizer;
      toggleRandomizerUI(isRandomizer);

      if (!isRandomizer) {
          pinnedOnlyCheckbox.checked = settings.pinnedOnly || false;
      }
      enabledCheckbox.checked = settings.enabled !== false;
    });
  }

  function saveSettings() {
    const min = parseInt(minIntervalInput.value, 10);
    const max = parseInt(maxIntervalInput.value, 10);
    
    if (isNaN(min) || isNaN(max) || min <= 0 || max <= 0 || min > max) {
      alert('Please enter valid positive numbers. Min interval must not be greater than max interval.');
      return;
    }

    const settings = {
      urls: targetUrls,
      minInterval: min,
      maxInterval: max,
      pinnedOnly: pinnedOnlyCheckbox.checked,
      enabled: enabledCheckbox.checked,
      randomizerEnabled: randomizerEnabledCheckbox.checked,
      randomUrlList: randomUrls
    };

    chrome.storage.sync.set(settings, showSavePopup);
  }

  // --- Event Listeners ---
  addUrlButton.addEventListener('click', () => {
    const newUrl = newUrlInput.value.trim();
    if (newUrl && !targetUrls.includes(newUrl)) {
      targetUrls.push(newUrl);
      renderUrlList(urlListContainer, targetUrls);
      newUrlInput.value = '';
    }
  });

  addCurrentUrlButton.addEventListener('click', () => {
    chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
      if (tabs[0]?.url && !targetUrls.includes(tabs[0].url)) {
        targetUrls.push(tabs[0].url);
        renderUrlList(urlListContainer, targetUrls);
      }
    });
  });

  addRandomUrlButton.addEventListener('click', () => {
    const newUrls = newRandomUrlTextarea.value.split('\n').map(u => u.trim()).filter(Boolean);
    newUrls.forEach(url => {
        if (!randomUrls.includes(url)) randomUrls.push(url);
    });
    renderUrlList(randomUrlDisplayList, randomUrls);
    newRandomUrlTextarea.value = '';
  });
  
  addCurrentRandomUrlButton.addEventListener('click', () => {
    chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
      if (tabs[0]?.url && !randomUrls.includes(tabs[0].url)) {
        randomUrls.push(tabs[0].url);
        renderUrlList(randomUrlDisplayList, randomUrls);
      }
    });
  });

  randomizerEnabledCheckbox.addEventListener('change', (e) => toggleRandomizerUI(e.target.checked));
  saveButton.addEventListener('click', saveSettings);
  
  function showSavePopup() {
    const popup = document.getElementById('save-popup');
    popup.classList.add('visible');
    if (savePopupTimeout) clearTimeout(savePopupTimeout);
    savePopupTimeout = setTimeout(() => popup.classList.remove('visible'), 1500);
  }

  loadSettings();
});